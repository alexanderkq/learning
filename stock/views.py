...
def stock_sell(request, pk):
    ...
    if form.is_valid():
        ...
        if amount > acc_stock.amount:
            form.add_error('amount', 'Недостаточно акций для продажи')
        else:
            current_cost = acc_stock.average_buy_cost * acc_stock.amount
            ...
            if total_amount == 0:
                # Если количество акций становится 0, обнуляем среднюю стоимость
                acc_stock.average_buy_cost = 0
            else:
                acc_stock.average_buy_cost = total_cost / total_amount

            acc_currency, _ = AccountCurrency.objects.get_or_create(account=request.user.account,
...
def account(request):
    ...
    if currencies is None:
        ...
        cache.set(f'currencies_{request.user.username}', currencies, 1)
    ...
    if stocks is None:
        ...
        cache.set(f'stocks_{request.user.username}', stocks, 1)
